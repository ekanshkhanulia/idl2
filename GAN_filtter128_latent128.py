...
# save_path   = "/content/drive/MyDrive/faces_20k_64x64.npy"
...
# # SAVE
# np.save(save_path, X)
# print("\n Saved .npy dataset at:")
# print(save_path)

# %%
# def make_to_npy("whole dataset"):
#   for in img_celba_folder:
#     #resize to 64*62

#     if rgb >3:
#       convert it into 3 rgb

#     save it into python list
...
def load_real_samples(scale=False):
    ...
    #X = np.load("/content/drive/MyDrive/faces_20k_64x64.npy",  fix_imports=True,encoding='latin1')[:20000, :, :, :] #fix differences from python2 to 3 # slice and take 20000 images and all te rows and column
    X=np.load("faces_20k_64x64.npy")
...
#grid_plot(dataset[:9], name='Fliqr dataset (64x64x3)', n=3)
...
def build_conv_net(in_shape, out_shape, n_downsampling_layers=4, filters=128, out_activation='sigmoid'):  # basically an encoder
    """
    Build a basic convolutional network
    """
    default_args=dict(kernel_size=(3,3), strides=(2,2), padding='same', activation='relu')

    input = tf.keras.Input(shape=in_shape) # image as an input

    x = Conv2D(filters=filters, name='enc_input', **default_args)(input) #first conv2d layer defines inital feature extraction form the raw image ,and hence sperate
...
#Interpolation :
 ...
 #pick 2 random vectors from the latent space
 #find intermediate points in between
 # then feed those points to the decoder
 #and it will output the images

# %%
import numpy as np
import matplotlib.pyplot as plt
#choose 2 random latent vector

z1=np.random.randn(latent_dim)
z2=np.random.randn(latent_dim)

#number of interpolation step:
num_steps=7 #total image will be 9
...
generated_images=generator.predict(interpolated_vectors)

#grid_plot(generated_images,name="GAN Latent Space Interpolation",n=3)
